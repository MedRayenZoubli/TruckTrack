...
const string TRUCK_ID = "TRUCK-001";
...
mqttClient.ApplicationMessageReceivedAsync += async e =>
{
    if (e.ApplicationMessage.Topic == $"trucks/{TRUCK_ID}/status")
    {
        ...
        if (newStatus == "ALERT")
        {
            Console.WriteLine($"     WARNING: Truck drifting! Return to delivery zone!");
        }
...
using (var httpClient = new HttpClient())
{
    while (true)
    {
        lat += (Random.Shared.NextDouble() - 0.5) * 0.005;
        lon += (Random.Shared.NextDouble() - 0.5) * 0.005;
        ...
        try
        {
            ...
            Console.WriteLine($" {TRUCK_ID}: ({truck.latitude}, {truck.longitude}) | Status: {currentStatus}");
        }
        catch (Exception ex)
        {
